...
class HS2_DB
{
	...
	function lastError() {
		...
		return 'MySQL error: (' . mysqli_errno($this->link) . ') ' . mysqli_error($this->link) .
	...
	function open($host, $db, $login, $pass, $char_set = 'utf8')
	{
		try
		{
			$this->link = @mysqli_connect($host, $login, $pass);
			$this->checkLink();
	...
	{
		$this->checkLink();
		return @mysqli_affected_rows($this->link); // '-1' if error / '2' if replace
	}

	function rowCount()
	{
		$this->checkLink();
		return @mysqli_num_rows($this->link); // 'false' if error
	}

	function colCount()
	{
		$this->checkLink();
		return @mysqli_num_fields($this->link); // 'false' if error
	}

	function newID()
	{
		$this->checkLink();
		return @mysqli_insert_id($this->link); // 'false' if error
	}
